...
namespace ThBIMServer.Ifc2x3
{
    public static class ThIFC2x32ProtoBufFactory
    {
        ...
        public static ThTCHProjectData CreateTCHProject(this IfcProject project)
        {
            ...
            var buildingStoreys = project.Sites.First().Buildings.First().BuildingStoreys;
            foreach (var storey in buildingStoreys)
            {
                var floorName = ((Xbim.Ifc2x3.Kernel.IfcRoot)storey).Name.Value;
        ...
        private static ThTCHWallData WallDataEntityToTCHWall(this IfcWall ifcWall)
        {
            ...
            newWall.BuildElement.EnumMaterial = ((Xbim.Ifc2x3.MaterialResource.IfcMaterialLayerSetUsage)ifcWall.Material).ForLayerSet.LayerSetName;
            ...
            if (ifcWall.Representation.Representations.First().Items[0] is IfcExtrudedAreaSolid areaSolid)
            {
                ...
                newWall.BuildElement.Outline = ((Xbim.Ifc2x3.ProfileResource.IfcArbitraryClosedProfileDef)areaSolid.SweptArea).OuterCurve.ToTCHMPolygon();
            }
